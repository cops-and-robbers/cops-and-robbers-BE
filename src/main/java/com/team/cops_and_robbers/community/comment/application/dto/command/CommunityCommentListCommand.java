...
/** 커서 페이징은 1depth 댓글에만 적용한다. 답글은 그 페이지에 속한 것을 전부 함께 내려준다. */
public record CommunityCommentListCommand(
        Long postId,
        Long cursor,
        int size
) {
    ...
    public CommunityCommentListCommand {
        if (size < 1 || size > MAX_SIZE) {
            throw new ApplicationException(CommonException.INVALID_QUERY_PARAMETER);
        }
    }

    public static CommunityCommentListCommand of(Long postId, Long cursor, int size) {
        return new CommunityCommentListCommand(postId, cursor, size);
    }
