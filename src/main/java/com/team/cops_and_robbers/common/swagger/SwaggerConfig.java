    ...
    public OpenAPI openAPI() {
        ...
        Info info = new Info()
                ...
                .description("""
                        ...
                        ### 🛠 변경 — 인게임 채팅 FCM 푸시 방식 조정 (부하테스트 반영)
                        - 푸시 문구가 고정된다: title="새 채팅", body="새 채팅이 도착했습니다."
                          (기존 title=발신자 닉네임 / body=메시지에서 변경). 여러 채팅이 묶여 나가므로 내용 대신 신호만 준다
                        - 한 게임의 채팅 푸시를 (게임 + scope) 단위로 30초에 1건만 보낸다.
                          collapse key(chat:{gameId})로 기기 알림 서랍에서 한 게임의 채팅 알림은 1개로 합쳐진다
                        - 수신자 명단을 DB 대신 인게임 참가자 캐시에서 읽는다. 발신자·팀·푸시동의 필터 동작은 동일하다
                        - data={type, gameId, scope}와 "서버는 화면 보는 사람을 걸러내지 않는다(앱이 게임 화면에서 억제)"는 그대로다
                        - **앱 영향** — payload 구조·data 키가 동일해 코드 변경은 없다.
                          다만 알림 문구가 바뀌고 여러 알림이 하나로 합쳐지므로 화면에 보이는 동작이 달라진다
                        ...
                        ### ✨ 신규 — 인게임 채팅 FCM 푸시
                        - 인게임 채팅을 보내면 참가자에게 FCM 푸시가 간다. 커뮤니티 채팅과 같은 구조다
                          - 전체 채팅(scope=ALL)은 발신자를 뺀 전원, 팀 채팅(scope=TEAM)은 발신자와 같은 팀만 받는다
                          - 게임 푸시를 끈 사용자(allowGamePush=false)는 제외된다
                        - 푸시 payload: title=발신자 닉네임, body=메시지,
                          data={type: "CHAT", gameId, scope}
                        - **앱 작업 필요** — 게임 화면을 보고 있을 때는 표시하지 않아야 한다.
                          서버는 누가 게임 화면을 보고 있는지 알 수 없어 걸러내지 않는다 (커뮤니티 채팅과 동일)

                        ### 🐛 수정 — 이벤트 게임 참가자가 채팅·핑·푸시를 못 받던 문제
                        - 이벤트 게임은 진행 중(IN_PROGRESS)에 초대 코드로 들어오는데, 인게임 참가자 캐시는
                          게임 시작 시점에 한 번만 적재돼서 그 뒤에 들어온 사람이 캐시에 없었다
                          - 캐시에 없으면 채팅 전송이 예외로 실패하고, 핑은 조용히 버려지고,
                            시스템 이벤트(체포·탈옥·게임 종료 등) FCM 대상에서도 빠진다
                          - 채팅 FCM 수신자는 DB 에서 뽑으므로 캐시와 무관하다. 다만 보낸 사람이 캐시에 없으면
                            채팅 자체가 실패해 그 메시지의 푸시도 나가지 않는다
                          - 그동안은 그 방의 누군가가 다시 로그인해 캐시가 통째로 재적재될 때만 우연히 들어왔다
                        - 이벤트 입장 시점에 해당 참가자를 캐시에 바로 넣는다
                        - 캐시 TTL 이 입장마다 갱신된다. 사람이 계속 들어오는 이벤트 방에서 중간에 만료되지 않는다
                        - 앱 영향 없다

                        ## v2.33.0 업데이트 내역

                        ### 🛠 변경 — 게임 중 퇴장한 참가자를 게임 기록에 보존
                        - 위치정보법 제16조 제2항 확인자료의 "제공받은 자" 명단 대응이다
                        - 게임 기록(game_results)을 게임 종료 시점이 아니라 게임 시작 시점에 만든다
                          - 참가자 명단도 시작 시점에 함께 남긴다. 좌표 제공이 시작되는 시점이 곧 기록 시점이다
                          - 승자·종료 사유·인원 집계·진행 시간은 종료될 때 같은 행에 채운다
                        - 게임 중 퇴장하면 참가자 명단에서 지우지 않고 leftAt 에 퇴장 시각만 찍는다
                          - 그동안은 퇴장 시 participants 행을 지운 뒤 종료 시점에 명단을 떠서, 중간에 나간 사람이
                            기록에 아예 없었다. 경찰이 전원 퇴장한 몰수패는 "제공받은 자"가 공집합이었다
                        - 어드민 게임 기록 참가자(AdminGameHistoryParticipant)에 leftAt 노출
                        - 인원 집계(totalPoliceCount 등)는 종료 시점에 남아 있던 인원 그대로다
                          - 앱 결과 화면의 "남은 도둑 수" 계산이 흔들리지 않도록 의도적으로 두었다
